...
//		cout << "Read " << pair_name << "_no_object" << to_string(i) << ".png" << endl;
	...
	double sum, max, average, temp1, temp2;
	for(int j = 0; j < row; j++) {
		for(int i = 0; i < col; i++) {
			sum = 0, max = 0, average = 0;
			for(int h = 0; h < N_IMAGES; h++) {
				sum += (double)src_img[h].at<uchar>(j, i);
				if((max < (double)src_img[h].at<uchar>(j, i)) && ((double)src_img[h].at<uchar>(j, i) != 255))
					max = (double)src_img[h].at<uchar>(j, i);
			}
			average = sum / N_IMAGES;
			
			temp1 = 0, temp2 = 0;
			for(int h = 0; h < N_IMAGES; h++) {
				temp1 += pow(((double)src_img[h].at<uchar>(j, i) - average), 2);
			}
			temp1 /= N_IMAGES;
			temp2 = sqrt(temp1);
//			cout << temp2 << endl;
			...
			if(temp2 <= 2) {	
				temp.at<uchar>(j, i) = (uchar)(average * 0.9);
			} else if((2 < temp2) && (temp2 <= 5)) {
				temp.at<uchar>(j, i) = (uchar)average;
			} else if((5 < temp2) && (temp2 <= 20)) {
				temp.at<uchar>(j, i) = (uchar)((average+max)/2); 
			} else if(10 < temp2) {	
